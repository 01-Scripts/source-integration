...
<?php
	...
	public function import_commits( $p_repo, $p_commit_ids, $p_branch='' ) {
		...
		while( count( $s_parents ) > 0 && $s_counter < 200 ) {
			...
			mail("info@01-scripts.de","Service hook is importing $t_commit_id ... ", "repos/$t_username/$t_reponame/commits/$t_commit_id");
	...
	private function json_commit_changeset( $p_repo, $p_json, $p_branch='' ) {
		...
		if ( !SourceChangeset::exists( $p_repo->id, $p_json->sha ) ) {
			...
			$t_changeset = new SourceChangeset( $p_repo->id, $p_json->sha, $p_branch,
				$p_json->commit->author->date, $p_json->commit->author->name, utf8_decode($p_json->commit->message) );
